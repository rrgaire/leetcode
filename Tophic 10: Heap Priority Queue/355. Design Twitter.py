...
class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        if userId not in self.postdict:
            self.postdict[userId] = []
        self.postdict[userId].append([self.id, tweetId])
        self.id += 1
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        res = []
        posts = []
        if userId in self.postdict:
            for post in self.postdict[userId]:
                posts.append(post)
        if userId in self.followdict:
            for user in self.followdict[userId]:
                if user in self.postdict:
                    for post in self.postdict[user]:
                        posts.append(post)
        posts = [(-i, t) for (i, t) in posts]
        heapq.heapify(posts)
